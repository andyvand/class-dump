//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "TBaseBrowserWindowController.h"

#import "BrowserContainerDelegate-Protocol.h"
#import "BrowserContainerSearching-Protocol.h"
#import "BrowserContainerTargeting-Protocol.h"
#import "NSDraggingDestination-Protocol.h"
#import "NSMenuItemValidation-Protocol.h"
#import "NSTabDraggingDestination-Protocol.h"
#import "NSTabDraggingWindowDestination-Protocol.h"
#import "NSToolbarItemValidation-Protocol.h"
#import "NSTouchBarDelegate-Protocol.h"
#import "NSWindowDelegate-Protocol.h"
#import "NSWindowRestoration-Protocol.h"
#import "TToolbarControllerDelegateProtocol-Protocol.h"

@class NSDate, NSString, NSTimer, NSWindow, TBackupContainerDelegate, TBrowserContainerController, TBrowserContentViewController, TBrowserWindowPlusTabDraggingDelegate, TContainerLayoutManager, TNodeViewSettings, TToolbarController;

@interface TBrowserWindowController : TBaseBrowserWindowController <TToolbarControllerDelegateProtocol, NSWindowRestoration, NSTouchBarDelegate, NSWindowDelegate, NSDraggingDestination, NSMenuItemValidation, NSToolbarItemValidation, NSTabDraggingDestination, NSTabDraggingWindowDestination, BrowserContainerTargeting, BrowserContainerSearching, BrowserContainerDelegate>
{
    TToolbarController *_toolbarController;
    struct TKeyValueBinder _activeContainerBinder;
    struct TNSWeakPtr<TTouchBar> _activeTouchBar;
    struct TNSWeakPtr<TCustomTouchBarItem> _searchLabelTouchBarItem;
    struct TNSWeakPtr<TCustomTouchBarItem> _searchScopeTouchBarItem;
    struct TKeyValueObserver _firstResponderObserver;
    NSTimer *_springToFrontTimer;
    struct CGRect _springAnchorRect;
    struct OpaqueCoreDragHandler *_springTrackingHandler;
    struct CGRect _springRememberedFrame;
    long long _springRememberedWindowWeWereBehind;
    _Bool _springCloseWhenDragLeavesWindow;
    TBackupContainerDelegate *_initTimeBackupDelegate;
    TBrowserWindowPlusTabDraggingDelegate *_plusTabDraggingDelegate;
    _Bool _initingWindow;
    _Bool _animatingStatusBar;
    struct CGRect _nonFullScreenWindowFrame;
    struct CGRect _fullScreenWindowFrame;
    NSWindow *_coverWindow;
    shared_ptr_656241d2 _fullScreenAnimationInfo;
    _Bool _shouldCaptureImagesInline;
    function_e88ba8d7 _fullScreenExitCallback;
    struct vector<TNotificationCenterObserver, std::allocator<TNotificationCenterObserver>> _observers;
    TContainerLayoutManager *_containerLayoutManager;
    struct TKeyValueObserver _tabbedWindowsObserver;
    _Bool _isSearching;
    _Bool _enteringFullScreen;
    _Bool _exitingFullScreen;
    TBrowserContainerController *_activeContainer;
    NSDate *_windowCreationTime;
    NSString *_iTunesDeviceIdentifier;
}

+ (id)instanceMethodSignatureForSelector:(SEL)arg1;
+ (_Bool)shouldForwardSelectorToContainer:(SEL)arg1;
+ (_Bool)shouldShowStatusBar;
+ (_Bool)shouldShowTabView;
+ (_Bool)windowWillBeAddedAsTab:(long long)arg1 existingWindow:(id)arg2 forceNewWindow:(_Bool)arg3;
+ (double)windowStaggerOffset;
+ (id)windowStateForNode:(const struct TFENode *)arg1 forceContainer:(_Bool)arg2;
+ (id)defaultWindowStateForNode:(const struct TFENode *)arg1;
+ (void)restoreWindowWithIdentifier:(id)arg1 state:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (id).cxx_construct;
- (void).cxx_destruct;
@property(retain, nonatomic) NSString *iTunesDeviceIdentifier; // @synthesize iTunesDeviceIdentifier=_iTunesDeviceIdentifier;
@property(readonly, nonatomic, getter=isExitingFullScreen) _Bool exitingFullScreen; // @synthesize exitingFullScreen=_exitingFullScreen;
@property(readonly, nonatomic, getter=isEnteringFullScreen) _Bool enteringFullScreen; // @synthesize enteringFullScreen=_enteringFullScreen;
@property(readonly, nonatomic) NSDate *windowCreationTime; // @synthesize windowCreationTime=_windowCreationTime;
@property(retain, nonatomic) TBrowserContainerController *activeContainer; // @synthesize activeContainer=_activeContainer;
@property(nonatomic) _Bool isSearching; // @synthesize isSearching=_isSearching;
@property(nonatomic) _Bool initingWindow; // @synthesize initingWindow=_initingWindow;
- (_Bool)runResizeTestWithOptions:(id)arg1;
- (_Bool)runScrollTestWithOptions:(id)arg1;
- (_Bool)_prepareForTestWithOptions:(id)arg1;
- (id)_testContentURLCreateIfNeeded;
- (id)searchScopeViewController;
- (id)touchBar:(id)arg1 makeItemForIdentifier:(id)arg2;
- (id)makeTouchBar;
- (_Bool)performTabDragOperation:(id)arg1;
- (unsigned long long)tabDraggingUpdated:(id)arg1;
- (unsigned long long)tabDraggingEntered:(id)arg1;
- (id)destinationForTabDragOperation:(id)arg1;
- (void)setSidebarWidthForScripting:(double)arg1;
- (double)sidebarWidthForScripting;
- (void)springRemoveDragTrackingHandler;
- (void)springInstallDragTrackingHandler;
- (void)configureAccessoryViews;
- (void)tearDownPersistentViews;
- (_Bool)wantsPeriodicDraggingUpdates;
- (void)updateDraggingItemsForDrag:(id)arg1;
- (void)draggingEnded:(id)arg1;
- (void)concludeDragOperation:(id)arg1;
- (void)draggingExited:(id)arg1;
- (_Bool)performDragOperation:(id)arg1;
- (_Bool)prepareForDragOperation:(id)arg1;
- (unsigned long long)draggingUpdated:(id)arg1;
- (unsigned long long)draggingEntered:(id)arg1;
- (void)configureWindowForContentChange;
- (id)window:(id)arg1 draggingDelegateForTab:(id)arg2;
- (id)window:(id)arg1 draggedTypesForTab:(id)arg2;
- (void)windowDidEndSheet:(id)arg1;
- (void)windowWillBeginSheet:(id)arg1;
- (void)windowDidEndLiveResize:(id)arg1;
- (void)windowWillStartLiveResize:(id)arg1;
- (_Bool)window:(id)arg1 shouldDragDocumentWithEvent:(id)arg2 from:(struct CGPoint)arg3 withPasteboard:(id)arg4;
- (_Bool)windowShouldClose:(id)arg1;
- (void)windowDidResize:(id)arg1;
- (void)windowDidEndMove:(id)arg1;
- (void)sidebarWidthDidChange;
- (void)windowDidMove:(id)arg1;
- (void)windowDidChange;
- (void)windowDidBecomeMain:(id)arg1;
- (void)windowDidMiniaturize:(id)arg1;
- (struct CGRect)windowWillUseStandardFrame:(id)arg1 defaultFrame:(struct CGRect)arg2;
- (void)showOrHideScrollbars:(_Bool)arg1;
- (void)animateToOrFromFullScreenWithDuration:(double)arg1;
- (void)prepareDestinationWindowImage:(shared_ptr_656241d2)arg1;
- (void)prepareCoverWindowLayersAndAnimationInfo;
- (double)lastVisibleToolbarItemWidth;
- (_Bool)fullScreenAnimationInProgress;
- (void)prepareToAnimateToFullScreen:(_Bool)arg1;
- (void)prepareAnimateFromInfo:(void *)arg1;
- (void)prepareAnimateToInfo:(void *)arg1;
- (struct CGRect)titleBarRect;
- (void)prepareFullScreenAnimationInfo:(void *)arg1;
- (void)setShowsTitleBarViews:(_Bool)arg1;
- (void)prepareNonFullScreenAnimationInfo:(void *)arg1;
- (void)setExitFullScreenCallback:(const void *)arg1;
- (void)windowDidExitFullScreen:(id)arg1;
- (void)windowWillExitFullScreen:(id)arg1;
- (void)window:(id)arg1 startCustomAnimationToExitFullScreenWithDuration:(double)arg2;
- (id)customWindowsToExitFullScreenForWindow:(id)arg1;
- (void)window:(id)arg1 startCustomAnimationToEnterFullScreenWithDuration:(double)arg2;
- (id)prepareWindowsToAnimateToFullScreen:(_Bool)arg1 forWindow:(id)arg2;
- (id)customWindowsToEnterFullScreenForWindow:(id)arg1;
- (void)windowDidEnterFullScreen:(id)arg1;
- (void)windowWillEnterFullScreen:(id)arg1;
- (unsigned long long)window:(id)arg1 willUseFullScreenPresentationOptions:(unsigned long long)arg2;
- (struct CGSize)window:(id)arg1 willUseFullScreenContentSize:(struct CGSize)arg2;
- (_Bool)useDefaultFullScreenAnimation:(_Bool)arg1;
- (void)containerTitleSeparatorChanged:(id)arg1;
- (void)containerCloseContainer:(id)arg1;
- (void)containerConnectBackupDelegate:(id)arg1;
- (void)containerWindowDidChange:(id)arg1;
- (void)containerContentChanged:(id)arg1;
- (void)containerSelectionChanged:(id)arg1;
- (_Bool)containerInBrowseMode:(id)arg1;
- (pair_794fd06c)searchScopeTitlesAndScopeIndex;
- (void)searchWasChanged;
- (void)searchDidBeginWithScopes:(const void *)arg1 selectedScopeIndex:(unsigned int)arg2;
- (void)updateIsSearching;
- (id)toolbarHistorySearchTemplate;
- (void)toolbarDisplayModeChanged;
- (_Bool)toolbarMakeSearchFieldFirstResponder;
- (id)toolbarSearchField;
- (_Bool)toolbarSearchItemIsVisible;
- (id)containerQuerySearchUTIs:(id)arg1;
- (void)containerForgetSpringState:(id)arg1;
- (void)container:(id)arg1 adjustProposedTargetPath:(void *)arg2;
- (void)containerHistoryDidChange:(id)arg1;
- (void)containerTargetDidChange:(id)arg1;
- (void)containerTargetWillChange:(id)arg1;
- (void)containerViewStyleChanged:(id)arg1;
- (void)pathPopupSelectedWithNode:(struct TFENode)arg1;
- (_Bool)shouldEnablePathMenuItem:(id)arg1 forNode:(const struct TFENode *)arg2;
- (void)cmdSearchFromToolbar:(id)arg1;
- (_Bool)canSetFocusToSearchField;
- (void)cmdToggleStatusBar:(id)arg1;
- (_Bool)validateToggleStatusBar:(id)arg1;
- (_Bool)validateRunToolbarCustomizationPalette:(id)arg1;
- (void)runToolbarCustomizationPalette:(id)arg1;
- (void)refreshToolbarItemsForSyncExtensionChange;
- (void)handlePendingAddOrRemoveToolbarItemsForSyncExtensions;
- (void)cmdToggleToolbar:(id)arg1;
- (_Bool)validateToggleToolbar:(id)arg1;
- (void)cmdClose:(id)arg1;
- (_Bool)respondsToSelector:(SEL)arg1;
- (id)forwardingTargetForSelector:(SEL)arg1;
- (_Bool)validateToolbarItem:(id)arg1;
- (_Bool)containerValidateToolbarItem:(id)arg1;
- (_Bool)validateTargetForMenuItem:(id)arg1;
- (void)copySearchFieldFromOther:(id)arg1;
- (void)addingSearchToolbarItem:(id)arg1;
- (void)searchFieldTextChanged;
- (void)configureStatusBar:(_Bool)arg1;
- (void)toggleStatusBarVisibility:(_Bool)arg1;
- (void)resizeForStatusBarChange:(struct CGSize)arg1;
- (double)statusBarHeightDeltaForVisibility:(_Bool)arg1;
- (_Bool)isPathBarVisible;
- (_Bool)isStatusBarVisible;
- (_Bool)isToolbarVisible;
- (id)toolbar;
- (void)hideTabViewForBackup;
- (void)showTabViewForBackup;
- (void)cmdToggleTabView:(id)arg1;
- (void)setDefaultTabVisibility;
- (void)newWindowForTab:(id)arg1;
- (void)iconAppearancePrefChanged:(_Bool)arg1;
- (id)backupDelegate;
- (id)activeBrowserViewController;
- (id)activeBaseBrowserViewController;
- (struct CGRect)frameForContainer;
- (_Bool)isTabBarVisible;
- (_Bool)isSprungAwayFromOriginalTarget;
- (void)revertToPreviousSpringState;
- (_Bool)windowIsSpringing;
- (void)setCloseWhenDragLeavesWindow;
- (void)forgetSpringState;
- (void)springWindowIntoView;
- (struct CGRect)rememberSpringState;
- (void)springDragInsideWindow:(struct OpaqueCoreDrag *)arg1;
- (void)springDragLeaveWindow:(struct OpaqueCoreDrag *)arg1;
- (void)springDragEnterWindow:(struct OpaqueCoreDrag *)arg1;
- (void)springUpdateAnchorRect:(struct OpaqueCoreDrag *)arg1;
- (void)stopSpringToFrontTimer;
- (void)animateToBrowseMode:(_Bool)arg1;
- (struct CGImage *)copyCGSWindowImageIncludeShadow:(_Bool)arg1;
- (void)createShadowImage:(struct CGImage **)arg1 andFrame:(struct CGRect *)arg2 andContentFrame:(struct CGRect *)arg3 forWindow:(id)arg4;
- (void)closeIfTargetedToDomain:(id)arg1;
- (void)closeIfDataSeparatedTarget;
- (void)updateICloudTabs;
- (void)configureToolbarForMode:(_Bool)arg1;
- (void)configureWindowForFullScreen:(_Bool)arg1;
- (void)setInBrowseModeAnimated:(_Bool)arg1;
- (void)setInBrowseMode:(_Bool)arg1;
- (_Bool)inBrowseMode;
- (void)resizeWindowWithSize:(struct CGSize)arg1;
- (void)setWindowFrame:(struct CGRect)arg1;
- (_Bool)shouldShowRecents;
- (_Bool)isDesktop;
- (long long)windowNumber;
- (id)browserWindow;
- (struct TFENode)fetchWindowProxyNode;
- (void)adjustWindowProxyNode:(struct TFENode *)arg1;
- (_Bool)shouldStaggerWindowPositionsForWindowState:(id)arg1;
- (struct CGRect)finderScriptingContentRectForFrameRect:(const struct CGRect *)arg1;
- (struct CGRect)finderScriptingFrameRectForContentRect:(const struct CGRect *)arg1;
- (unsigned int)finderScriptingModelKind;
- (struct TFENode)target;
- (_Bool)shouldCascadeWindows;
- (void)configureWindowForTargetChange;
- (void)configureWindowWithState:(id)arg1 targetPath:(const void *)arg2 spawnOrigin:(shared_ptr_3b64c573)arg3;
- (void)unlockOverlayScrollerStateOnMainThread;
- (void)configureWindowFrameWithState:(id)arg1;
- (struct CGRect)forceFrameToMainScreen:(struct CGRect)arg1;
- (void)configureWindowForBackup;
- (struct CGRect)pinWindowFrame:(struct CGRect)arg1 toScreenContainingPoint:(struct CGPoint)arg2 includingMouse:(_Bool)arg3;
- (_Bool)windowStateIsValid:(id)arg1;
- (id)windowState;
- (id)containerWindowState:(id)arg1;
- (id)restorableWindowState:(_Bool)arg1;
- (id)windowStateWithContainer:(id)arg1 forFinderWindowRestore:(_Bool)arg2;
- (void)saveWindowState;
- (struct TString)pathPrettyStringForNode:(struct TFENode)arg1;
- (struct TString)titleForICloudContainer:(const struct TFENode *)arg1;
- (struct TFENodeVector)popupPathForNode:(const struct TFENode *)arg1;
- (_Bool)nodeIsAnICloudRoot:(const struct TFENode *)arg1;
- (int)cloudFolderTypeForNode:(const struct TFENode *)arg1;
- (void)aboutToTearDown;
- (void)dealloc;
- (void)effectiveAppearanceChanged;
- (id)restorableStateRepresentedURL;
- (void)window:(id)arg1 willEncodeRestorableState:(id)arg2;
- (void)drawCompletelyIntoBackBuffer;
- (void)makeKeyAndOrderFront;
- (void)searchFieldCancel;
@property(retain, nonatomic) TNodeViewSettings *viewSettings;
- (int)viewStyle;
- (void)tabbedWindowsDidChange;
- (id)composedWindowState;
- (void)windowDidLoad;
- (Class)browserContainerClass;
@property(retain) TBrowserContentViewController *contentViewController;
- (id)initWithState:(id)arg1;
- (id)initWithTargetPath:(const void *)arg1 forceContainer:(_Bool)arg2 spawnOrigin:(shared_ptr_3b64c573)arg3 desiredLocation:(struct CGPoint)arg4 creationState:(id)arg5;
- (id)initWithTargetPath:(const void *)arg1 forceContainer:(_Bool)arg2 spawnOrigin:(shared_ptr_3b64c573)arg3 desiredLocation:(struct CGPoint)arg4;
- (id)initForBackupWithState:(id)arg1 targetPath:(const void *)arg2 forSnapshotGenerator:(_Bool)arg3 withTMTarget:(const void *)arg4;
- (id)initWithState:(id)arg1 targetPath:(const void *)arg2 spawnOrigin:(shared_ptr_3b64c573)arg3;
- (void)completeInitialization:(id)arg1 targetPath:(const void *)arg2 spawnOrigin:(shared_ptr_3b64c573)arg3;
- (id)init;
- (id)windowNibName;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
// Preceding property had unknown attributes: ?
// Original attribute string: T@"NSString",?,R,C

@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

