//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "TTextView.h"

@class NSWindow, TShrinkToFitController, TShrinkToFitWindow;

@interface TShrinkToFitTextView : TTextView
{
    TShrinkToFitWindow *_stfWindow;
    NSWindow *_parentWindow;
    struct CGRect _maxGlobalFrame;
    _Bool _textGrowsUpwards;
    unsigned long long _maxUniChars;
    long long _maxNumLines;
    struct TNSWeakPtr<TShrinkToFitController> _weakSTFController;
    double _srcGlobalBaseline;
}

- (id).cxx_construct;
- (void).cxx_destruct;
@property(nonatomic) long long maxNumLines; // @synthesize maxNumLines=_maxNumLines;
@property(nonatomic) unsigned long long maxUniChars; // @synthesize maxUniChars=_maxUniChars;
@property(nonatomic) _Bool textGrowsUpwards; // @synthesize textGrowsUpwards=_textGrowsUpwards;
@property(readonly, nonatomic) double srcGlobalBaseline; // @synthesize srcGlobalBaseline=_srcGlobalBaseline;
@property(readonly, nonatomic) struct CGRect maxGlobalFrame; // @synthesize maxGlobalFrame=_maxGlobalFrame;
@property(retain, nonatomic) NSWindow *parentWindow; // @synthesize parentWindow=_parentWindow;
- (void)quickLookWithEvent:(id)arg1;
- (id)_immediateActionAnimationControllerForCharacterAtIndex:(unsigned long long)arg1 withRecognizer:(id)arg2;
- (id)accessibilityActionNames;
- (id)accessibilityAttributeValue:(id)arg1;
- (id)accessibilityAttributeNames;
- (void)setMaxSize:(struct CGSize)arg1;
- (double)lastBaselineOffsetFromBottom;
- (double)firstBaselineOffsetFromTop;
- (pair_2447b6e7)lineFragmentAndBaselineForCharAtIndex:(unsigned long long)arg1;
- (void)setMinSize:(struct CGSize)arg1;
- (void)setMaxGlobalFrame:(const struct CGRect *)arg1 andGlobalBaseline:(double)arg2;
- (double)calcGlobalBaselineFromTop;
- (void)setFont:(id)arg1;
- (void)doCommandBySelector:(SEL)arg1;
- (_Bool)shouldChangeTextInRange:(struct _NSRange)arg1 replacementString:(id)arg2;
- (_Bool)isEditing;
- (void)stopEditing:(_Bool)arg1;
- (void)startEditing;
- (void)didChangeText;
- (void)setString:(id)arg1;
- (void)setConstrainedFrameSize:(struct CGSize)arg1;
- (void)configureAttributes;
- (void)configureSTFWindow;
@property(nonatomic) __weak TShrinkToFitController *stfController;
- (void)dealloc;
- (id)init;

@end

