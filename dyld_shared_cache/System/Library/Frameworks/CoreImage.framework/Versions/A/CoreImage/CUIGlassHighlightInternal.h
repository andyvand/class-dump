//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <CoreImage/CIFilter.h>

@class CIColor, CIImage, CIVector, NSNumber;

__attribute__((visibility("hidden")))
@interface CUIGlassHighlightInternal : CIFilter
{
    CIImage *inputImage;
    NSNumber *inputHeight;
    NSNumber *inputInset;
    NSNumber *inputSpread;
    NSNumber *inputBiasAmount;
    NSNumber *inputCurvature;
    CIVector *inputAngleSinCos;
    CIColor *inputColor;
    NSNumber *inputSDFScaleFactor;
    NSNumber *inputSDFZeroValue;
    CIVector *inputCenter;
}

- (void);
- (void);
- (void);
- (void);
- (void);
- (void);
- (id);
- (id);
- (id);
- (id);
- (id);
- (id);
- (void);
- (id);
- (id);
- (void);
- (id);
- (void);
- (id);
- (void);
- (id);
- (void);
- (id);
- (id)t|||||||awt}\vstttt[f]hif\nn}myuNbWn:RD_ETG\S^Vb]oms||||ipppjjjjUc\ffd\f[`Ug3WJo_tp[e[m7O?_acYdppppssssee[mX_Smkvq{Vqlbjaufjbn^niowwwwaxv~iwtj{v}{££££¤¤¤¤¦¦§§§§§§yiiiiaa^annnn||||oooo!>0S-<+Pafbkibjdnffbfkkjk[plxv}Qa\i:K=Vffcfªªª±±±±{O^SfGP@ZDUIbxwuxg|z|j{{}_rpyhwv}|}}}}ptozQcYrismyk¡¡¡¡u=VGj%A,X>P@W``\``ljl||||]sszF^WmUb\g^^\^AL@Q>J=SV`ZePjhr_mktyyyy}}}}ppppWWOX(:)H+=-JRVNXbbbb^^^^HTNVYYYYffffyyyy««««¥¥¥¥jcrkyttttuuuuixty{¦««¶¶¶¶¨¨¨¨wwpwepgue\ngwhmfo_nioPg_mnnnnttqtNnfso|Vpj}|^rqzfc[h\_UdZjdq]vsF^UqBRG_=H:WHZT`oooorrrrSe_kM_Tjorkv~x\ojxPc[pzwuz    kxtk}{ctoxrrmrxxsx¤¤¤¤««««££££}}}}vvvvZc^i<QE^XkgoUpnLVIdXUIX[[[[``^`N^VbRbZennnnw}|}YoiYlfz{{z{||{|{yw{zywzbtqyyyyywwwwfzx~¥¥¥¥£¢££]}xisk|PjdvSbZmyyyy~~~~ssss\d\lwwtws{wyn_|z~w~~~vvvvtrot~k|wo{§£§§·¼½½¾¿ÀÀ¦©®µ¢¥±¶¶¶¶°°°°r~ynxs~~_{x~~^tnz¢¢¢¢ªªªª}¢¦s{{{{nnnncd]dO^S`Y_Tf}y££££¼¼¼¼ÁÁÁÁ¶¶¶¶¡¡¡¡zQof`xsxxxxIYP]:I<RY[U[hhhhhpnpvvvvwwww /* Error: Ran out of types for this method. */;

// Remaining properties
@property(retain, nonatomic) CIVector *inputAngleSinCos; // @synthesize inputAngleSinCos;
@property(retain, nonatomic) NSNumber *inputBiasAmount; // @synthesize inputBiasAmount;
@property(retain, nonatomic) CIVector *inputCenter; // @synthesize inputCenter;
@property(retain, nonatomic) CIColor *inputColor; // @synthesize inputColor;
@property(retain, nonatomic) NSNumber *inputCurvature; // @synthesize inputCurvature;
@property(retain, nonatomic) NSNumber *inputHeight; // @synthesize inputHeight;
@property(retain, nonatomic) CIImage *inputImage; // @synthesize inputImage;
@property(retain, nonatomic) NSNumber *inputInset; // @synthesize inputInset;
@property(retain, nonatomic) NSNumber *inputSDFScaleFactor; // @synthesize inputSDFScaleFactor;
@property(retain, nonatomic) NSNumber *inputSDFZeroValue; // @synthesize inputSDFZeroValue;
@property(retain, nonatomic) NSNumber *inputSpread; // @synthesize inputSpread;

@end

