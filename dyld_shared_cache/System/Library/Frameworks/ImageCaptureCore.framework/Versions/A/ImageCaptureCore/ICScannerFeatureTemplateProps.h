...
- (void)z * scn_frame.clusterScale.z + scn_frame.clusterScale.w; 
   ...
   _lightingContribution.add_global_probe(u_radianceTexture, scn_frame.viewToCubeTransform, scn_frame.environmentIntensity);
    ...
    #if DEBUG_PIXEL
        switch (DEBUG_PIXEL) {
            case 1:(int)arg1 _output.color = float4(_surface.normal * 0.5f + 0.5f, 1.f); break;
...
- (void)REFLECTIVE)
 ...
 /* Error: Ran out of types for this method. */;
- (id)(u_clearCoatRoughnessTexture.sample(u_clearCoatRoughnessTextureSampler, _surface.clearCoatRoughnessTexcoord), USE_CLEARCOATROUGHNESS_TEXTURE_COMPONENT).r;
#else
 ...
 /* Error: Ran out of types for this method. */;
- (int)onprofile_node& scn_node = scn_nodes[USE_MULTIPLE_RENDERING * in.instanceID + in.sliceIndex];
#endif
#else
