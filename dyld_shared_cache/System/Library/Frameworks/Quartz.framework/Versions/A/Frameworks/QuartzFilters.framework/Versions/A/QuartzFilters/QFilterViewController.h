...
+ (void)ionFactor[1] = tessLevelInner[1];
#if !USE_PTVS_FACTORS
...
;
+ (long long)OsdGetTessLevelsUniform(
        tessLevel, 
        patchParam, 
        tessLevelOuter,
        tessLevelInner,
...
;
- (void)float4x4 modelViewProjectionMatrix,
    OsdPatchParamBufferSet osdBuffers
    )
{
...
;
- (void)Apache License for the specific
...
;
- (void)//   Unless required by applicable law or agreed to in writing, software
...
;
- (id)names, trademarks, service marks, or product names of the Licensor
//      and its affiliates, except as required to comply with Section 4(c) of
//      the License and to reproduce the content of the NOTICE file.
//
//   You may obtain a copy of the Apache License at
...
;
- (void)tchVertices[ID], osdBuffers.perPatchVertexBuffer[ControlID]);
}
...
;
- (void)fers.perPatchVertexBuffer[ControlID],
		osdBuffers);
...
;
- (void)threadgroup PatchVertexType* patchVertices,
	OsdPatchParamBufferSet osdBuffers
	)
{
	OsdComputePerPatchVertexGregory(
...
;
- (void)short3 clip1 = short3(clipPos.x > -clipPos.w,                   
    clipPos.y > -clipPos.w,                   
    clipPos.z > -clipPos.w);                  
    hullVertex.clipFlag = short3(clip0) + 2*short3(clip1);
#endif
...
;
- (id)PerVertexGregory(vertexId, position.xyz, hullVertex, osdBuffers);
...
;
- (void)rVertexGregory& hullVertex,
    int vertexId,
    float4x4 modelViewProjectionMatrix,
    OsdPatchParamBufferSet osdBuffers
    )
