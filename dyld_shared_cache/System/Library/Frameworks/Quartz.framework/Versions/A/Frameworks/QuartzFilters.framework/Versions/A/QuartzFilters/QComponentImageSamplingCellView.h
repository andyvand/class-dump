...
;
- (void)ssellationFactorsHalf& quadFactors
	)
{
    float4 tessLevelOuter = float4(0,0,0,0);
    float2 tessLevelInner = float2(0,0);
...
;
- (void)hVertexType* patchVertices,
...
;
- (void)= short3(clipPos.x > -clipPos.w,                   
    clipPos.y > -clipPos.w,                   
    clipPos.z > -clipPos.w);                  
...
;
- (id)OsdComputePerVertexGregory(vertexId, position.xyz, hullVertex, osdBuffers);
...
;
- (id)Trademarks. is deleted and replaced with://
...
- (void)hTransitionMask(patchParam);
...
;
- (id)auto transitionMask = OsdGetPatchTransitionMask(patchParam);
    float4 tessLevelMin = float4(1)
    + float4(((transitionMask & 8) >> 3),
             ((transitionMask & 1) >> 0),
             ((transitionMask & 2) >> 1),
