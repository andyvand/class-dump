...
;
+ (id)arameterization(domainCoord,
                                           tessOuterLo,
                                           tessOuterHi);
#else
    auto transitionMask = OsdGetPatchTransitionMask(patchParam);
...
;
- (id)tessLevelOuter = float4(0,0,0,0);
    float2 tessLevelInner = float2(0,0);
...
;
- (id)short3 clip0 = short3(clipPos.x < clipPos.w,                    
    clipPos.y < clipPos.w,                    
    clipPos.z < clipPos.w);                   
...
;
- (id)icense with the above modification is
...
;
- (_Bool)6. Trademarks. is deleted and replaced with:(int)arg1 //
...
//
//       http:(int)arg2 //www.apache.org/licenses/LICENSE-2.0
//
