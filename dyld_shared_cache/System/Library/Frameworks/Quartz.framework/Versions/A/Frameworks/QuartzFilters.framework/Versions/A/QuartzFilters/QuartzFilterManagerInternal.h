...
+ (_Bool)dPatchParamBufferSet osdBuffer,
...
;
+ (void)return ds_regular_patches(
		tessLevel,
#if !USE_PTVS_FACTORS
#if USE_STAGE_IN
		osdPatch.tessOuterHi,
...
;
- (_Bool)atchVertexType* patchVertices,
	device MTLQuadTessellationFactorsHalf& quadFactors
	)
{
    float4 tessLevelOuter = float4(0,0,0,0);
...
;
- (_Bool)float4x4 modelViewMatrix,
...
;
- (_Bool)OSD_USER_VARYING_PER_CONTROL_POINT(osdBuffers.vertexBuffer[vertexId], hullVertex);
}
...
;
- (id)short3 clip1 = short3(clipPos.x > -clipPos.w,                   
    clipPos.y > -clipPos.w,                   
    clipPos.z > -clipPos.w);                  
...
;
- (id)owing modification to it:(id)arg1 //   Section 6. Trademarks. is deleted and replaced with:(id)arg2 //
...
//
//       http:(id)arg3 //www.apache.org/licenses/LICENSE-2.0
//
...
;
- (id)compliance with the Apache License and the following modification to it:(id)arg1 //   Section 6. Trademarks. is deleted and replaced with://
//   6. Trademarks. This License does not grant permission to use the trade
//      names, trademarks, service marks, or product names of the Licensor
//      and its affiliates, except as required to comply with Section 4(c) of
//      the License and to reproduce the content of the NOTICE file.
...
- (void)osdBuffers.perPatchVertexBuffer + patchID * VERTEX_CONTROL_POINTS_PER_PATCH,
        osdBuffers.patchParamBuffer[patchID],
#endif
		domainCoord);
}
...
;
- (id)at2 tessLevelInner = float2(0,0);
