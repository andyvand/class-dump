...
+ (id)NG_PER_VERTEX(patchVertices[ID], osdBuffers.perPatchVertexBuffer[ControlID]);
}
...
;
- (id)See the Apache License for the specific
...
;
- (void)//       http://www.apache.org/licenses/LICENSE-2.0
//
//   Unless required by applicable law or agreed to in writing, software
//   distributed under the Apache License with the above modification is
//   distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
...
//   compliance with the Apache License and the following modification to it:(int)arg1 //   Section 6. Trademarks. is deleted and replaced with://
...
;
- (id)output.patchCoord = OsdInterpolatePatchCoord(UV, patchParam);
    output.position = P;
...
;
- (void)output.vSegments = vSegments;
#endif
