//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <SiriInstrumentation/SISchemaInstrumentationMessage.h>

@class ODDSiriSchemaODDFixedDimensions;

@interface ODDSiriSchemaODDAssistantVoicesDigestsReported : SISchemaInstrumentationMessage
{
    ODDSiriSchemaODDFixedDimensions *_fixedDimensions;
}

- (void);
- (void);
- (void);
- (_Bool);
- (unsigned long long);
- (void);
- (void);
- (void);
- (void);
- (id);
- (id);
- (id);
- (id);
- (void);
- (unsigned long long);
- (_Bool);
- (_Bool);
- (id);
- (id)qÞð1Â0@ù
× ;
- (id)deleteAppAssetPath;
- (id)eltaTimeInNsFromlastPHSReject;
- (id),V_getMentionedApps;
- (void)eDatas.integerPayload;

// Remaining properties
@property(retain, nonatomic) ODDSiriSchemaODDFixedDimensions *fixedDimensions; // @synthesize fixedDimensions=_fixedDimensions;

@end

