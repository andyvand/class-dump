...
- (void)d3)
    ...
    geometry.color = (need_vertex_color) ? in.color :(id)arg1 float4(1.f);
}
...
static void standard_initalize_geometry_post_tessellation(thread scn_patch_t&       in,
                                                          ...
                                                          float3                    patchCoord)
{
    geometry.position = float4(scn:(id)arg2:(CDUnknownBlockType)arg3 barycentric_mix(in.controlPoints[0].position, in.controlPoints[1].position, in.controlPoints[2].position, patchCoord), 1.0);
