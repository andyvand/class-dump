...
+ (id);
+ (id);
- (_Bool)DMeshElement}^{__C3DMeshSource}^{__C3DMesh}^v^v}{?=CfIC(?={?=ff}{?=f}{?=f}{?=Cb1b1})}C}16Q24C32C36@"SCNMTLResourceManager"40@"<SCNMTLDeformerInitComputeContext>"48;
- (id)rtexType = OsdPerPatchVertexGregoryBasis;
#else
                    ...
                    thread float3& N, thread float3& dNu, thread float3& dNv)
{
    ...
    q[ 5] = (d11 == 0.0) ? cv[3]  :(long long)arg1 (u*cv[3] + v*cv[4])/d11;
