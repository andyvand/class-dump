//
//     Generated by class-dump 3.5 (64 bit) — C++ symbol-derived view.
//

class std::__1::vector<ClipperLib::PolyNode*, std::__1::allocator<ClipperLib::PolyNode*>> {
public: // (access info unavailable from symbols alone)
    auto __append(unsigned long);
    auto __throw_length_error[abi:nqe210106]();
    auto __vallocate[abi:nqe210106](unsigned long);
    auto reserve(unsigned long);
    auto resize(unsigned long);
    void __init_with_size[abi:nqe210106]<ClipperLib::PolyNode**, ClipperLib::PolyNode**>(ClipperLib::PolyNode**, ClipperLib::PolyNode**, unsigned long);
};
