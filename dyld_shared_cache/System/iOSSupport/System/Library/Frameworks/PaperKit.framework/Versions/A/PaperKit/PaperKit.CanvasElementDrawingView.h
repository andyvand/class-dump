//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

@class NSArray, PKTiledView, UIView;

@interface PaperKit.CanvasElementDrawingView
{
    id canvasElementView;
    id liveStreamingDisplayLink;
    id liveStreamingStartTimeInterval;
    id liveErasingDrawing;
    id mathRecognitionDelegate;
    id latestEndOfStrokeTimestamp;
}

- (void);
- (_Bool);
- (void);
- (void);
- (id);
- (void);
- (void);
- (void);
- (_Bool);
- (id);
- (id);
- (id);
- (_Bool);
- (_Bool);
- (double);
- (void);
- (void);
- (void);
- (void);
- (void);
- (id);
- (_Bool);
- (_Bool);
- (void);
- (void);
- (void);
- (struct CGAffineTransform);
- (id);
- (_Bool);
- (id);
- (_Bool);
- (void);
- (_Bool);
- (id);
- (void);
- (void)ôNÿÿ;
- (void);

// Remaining properties
@property(nonatomic, copy) NSArray *additionalStrokes;
@property(nonatomic, readonly) UIView *attachmentContainerView;
@property(nonatomic, readonly) _Bool canBecomeFirstResponder;
@property(nonatomic, readonly) struct CGAffineTransform drawingTransform;
@property(nonatomic, readonly) _Bool isAtEndOfDocument;
@property(nonatomic, readonly) PKTiledView *tiledView;
@property(nonatomic, readonly) _Bool updateHeightAfterDrawing;

@end

