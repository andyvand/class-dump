...
- (id),V_rulerDistanceText;
- (void)dule_state, m.sent_or_received_off_grid, m.is_time_sensitive, m.ck_chat_id, m.index_state FROM message m WHERE   m.is_from_me = 1   AND m.is_sent = 0   AND m.error = 0   AND m.item_type = 0   AND m.schedule_type = 0   AND m.ROWID > ? ;
- (id);
- (id);
- (struct CGSize);
- (id)Level /= pow(2.0, (0 - 1));
...
- (void);
- (id)isBeingCopied;
- (void)setAdjustsImageWhenHighlighted:(id)arg1;
