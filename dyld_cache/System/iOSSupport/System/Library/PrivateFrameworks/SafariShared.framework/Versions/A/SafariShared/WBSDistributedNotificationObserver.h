...
- (void);
- (id)4;
- (void)add_local_probe(scn_lights[LightIndex(lid)], u_reflectionProbeTexture);
    }
   ...
   _lightingContribution.add_global_probe(u_radianceTexture, scn_frame.viewToCubeTransform, scn_frame.environmentIntensity);
    ...
    #if DEBUG_PIXEL
        switch (DEBUG_PIXEL) {
            case 1:(CDUnknownBlockType)arg1 _output.color = float4(_surface.normal * 0.5f + 0.5f, 1.f); break;
