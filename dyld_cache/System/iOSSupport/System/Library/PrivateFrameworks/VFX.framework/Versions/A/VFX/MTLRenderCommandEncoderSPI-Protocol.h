                            ...
                            int3 patchParam, thread float4& tessOuterLo, thread float4& tessOuterHi)
{
    ...
    // PERFOMANCE:(id <MTLAccelerationStructure>)arg1 we just need to pick the correct corner points from P, P1, P2
...
template<typename VertexType> //VertexType should be some type that implements float3 VertexType:(unsigned long long)arg2:GetPosition()
...
- (void)_IN_TEXCOORD6) && (OSD_TEXCOORD6_INTERPOLATION_MODE == OSD_PRIMVAR_INTERPOLATION_MODE_FACE_VARYING)
