//
//     Generated by class-dump 3.5 (64 bit) — C++ symbol-derived view.
//

class AppleGraphicsDeviceControl {
public: // (access info unavailable from symbols alone)
    AppleGraphicsDeviceControl(OSMetaClass const*);
    auto UserKernelTransfer(unsigned int, AGDCClientState_t*);
    auto _RESERVEDAppleGraphicsDeviceControl0();
    auto _RESERVEDAppleGraphicsDeviceControl1();
    auto _RESERVEDAppleGraphicsDeviceControl2();
    auto _RESERVEDAppleGraphicsDeviceControl3();
    auto _RESERVEDAppleGraphicsDeviceControl4();
    auto _RESERVEDAppleGraphicsDeviceControl5();
    auto _RESERVEDAppleGraphicsDeviceControl6();
    auto _RESERVEDAppleGraphicsDeviceControl7();
    auto _actionWrapper(OSObject*, void*, void*, void*, void*);
    auto actionWrapper(AGDCPlugin*, void*);
    auto doDeviceAttribute(unsigned int, unsigned long*, unsigned long, unsigned long*, unsigned long*);
    auto doPlugin_Command(unsigned int, unsigned long long, AppleGraphicsDeviceControl*);
    auto filtered_doDeviceAttribute(AppleGraphicsDeviceControl::agdc_filtered_api_t, unsigned int, unsigned long*, unsigned long, unsigned long*, unsigned long*, AGDCClientState_t*);
    auto getDeviceAttribute(unsigned int, unsigned long*, unsigned long*);
    auto getMetaClass() const;
    auto message(unsigned int, IOService*, void*);
    auto newUserClient(task*, void*, unsigned int, OSDictionary*, IOUserClient**);
    auto publishHdaGfxProperties();
    auto setDeviceAttribute(unsigned int, unsigned long*, unsigned long);
    auto start(IOService*);
    auto stop(IOService*);
    auto terminate(unsigned int);
    auto vendor_doDeviceAttribute(unsigned int, unsigned long*, unsigned long, unsigned long*, unsigned long*, AGDCClientState_t*);
    void * operator new(unsigned long);
    void operator delete(void*, unsigned long);
    ~AppleGraphicsDeviceControl();
};
